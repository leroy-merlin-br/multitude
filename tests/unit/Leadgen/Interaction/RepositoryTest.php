...
class RepositoryTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testShouldFindExisting()
    {
        // Arrange
        $repo = new Repository();
        $result = m::mock(Interaction::class);
    ...
    public function testShouldGetUnacknowledged()
    {
        // Arrange
        $repo = new Repository();
        $result = m::mock(CursorInterface::class);
        ...
        $this->resourceRepo->shouldReceive('where')
            ->once()
            ->with(['acknowledged' => false], m::any(), m::any())
            ->andReturn($result);

        // Assert
        $this->assertEquals($result, $repo->getUnacknowledged());
    }
}
